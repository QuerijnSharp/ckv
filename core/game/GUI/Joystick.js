...
export default class Joystick {
  ...
  init(canvas) {
    ...
    var _eventHandler = (event) => {
      ...
      for (let touch of event.changedTouches) {
        ...
        let distance = _this.distance(_this.posX, x, _this.posY, y); //use isPointInPath instead??
        if (distance <= _this.radius * 5) {
          ...
          if (distance <= _this.radius / 2) {
            ...
            let resultVector = Vector2.projectOntoCircle(
              center,
              vec2,
              _this.radius / 2
            );
  ...
  onTouch(event) {
    ...
    for (let touch of event.changedTouches) {
      ...
      let distance = _this.distance(this.posX, x, y, touch.pageY); //use isPointInPath instead??
